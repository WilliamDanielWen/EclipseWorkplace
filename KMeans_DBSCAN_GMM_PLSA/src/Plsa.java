...
public class Plsa {
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i=0;i<numoftopics;i++){
			// sort array beta[i]
			System.out.println("\n############### Topic "+(i+1)+" top 10 words are : ####################");

			List<ProbAndIndex> Beta_i =new ArrayList<ProbAndIndex>();
			for(int j=0;j<beta[i].length;j++){
				Beta_i.add(new ProbAndIndex(beta[i][j],j));

			}
			Beta_i.sort(new ProbAndIndexComparator());
			for(int k=0;k<10;k++){
				System.out.println(myPLSA.allWords.get(Beta_i.get(k).index)+" , Probability: "+Beta_i.get(k).prob);
			}

		}

		
		// output the NMI and cluster result
		...
		int[] groundTruethLabel={3,3,1,1,1,4,3,3,4,2,4,2,1,2,3,2,1,2,4,4};
		System.out.println("\nVenue cluster results are:");
		for(int i=0;i<DocSize;i++){

			List<ProbAndIndex> Theta_i =new ArrayList<ProbAndIndex>();
			for(int j=0;j<numoftopics;j++){
				Theta_i.add(new ProbAndIndex(theta[i][j],j));
			}
			Theta_i.sort(new ProbAndIndexComparator());// sort to get the highest probability

			/* use the cluster and groundTruethLabel[i] to calculate the NMI, 
			  coordinate has no meaning, so we set it as 0,0 */ 
			DataPoints venue_i=new DataPoints(0,0,groundTruethLabel[i]); 
			int highestProbIndex=Theta_i.get(0).index;
			// put venue_i with groundTruethLabel[i] into cluster[highestProbIndex]
			clusters.get(highestProbIndex).add(venue_i);
			dataSet.add(venue_i);
			System.out.println("Venue "+(i+1)+" belongs to topic "+(highestProbIndex+1));
...
class ProbAndIndex{
	double prob;
	int index;
	ProbAndIndex(double p, int i){
		prob=p;
		index=i;
	}
}

class ProbAndIndexComparator implements Comparator<ProbAndIndex>{

	public int compare(ProbAndIndex a, ProbAndIndex b) {
