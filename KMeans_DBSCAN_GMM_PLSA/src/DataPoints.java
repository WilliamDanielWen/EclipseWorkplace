...
public class DataPoints {
	...
	public static double calcNMI(double[][] nmiMatrix) {
		...
		for(int i=0; i<row-1; i++) {
			for(int j=0; j<col-1; j++) {
				...
				if(logPart != 0d) I += (nmiMatrix[i][j]/N)*Math.log(logPart);
			}
		}

		for(int j=0;j<col-1;j++){
			double logPart = nmiMatrix[row-1][j]/N;
			if(logPart != 0d) HC -= logPart * Math.log(logPart);
		}

		for(int i=0;i<row-1;i++){
			double logPart=nmiMatrix[i][col-1]/N;
			if(logPart != 0d) HOmega -= logPart * Math.log(logPart);
		}
	...
	public static void writeToFile(List<Set<DataPoints>> clusters, String fileName) {
