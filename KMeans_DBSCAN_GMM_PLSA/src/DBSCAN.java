...
public class DBSCAN {
	...
	public static void main(String[] args) {
		...
		int seed = 71;
		System.out.println("################ DBSCAN On dataset1 ################");
		...
		System.out.println("Min points :"+minPts);
		dbscan(dataSet,"dataset1");
		...
		System.out.println("Epsilon :"+e);
		System.out.println("Min points :"+minPts);
		dbscan(dataSet,"dataset3");
	}

	static double e =0.03;
	static int minPts = 5;
	...
	private static double getEpsilon(List<DataPoints> dataSet) {
		...
		for(int i=0; i < dataSet.size(); i++) {
			...
			// maybe distance.get(14) is the best?
			sumOfDist += distances.get(14); // default is  7
			distances.clear();
		}
		return sumOfDist/dataSet.size();
	}
	
	private static void dbscan(List<DataPoints> dataSet, String dataSetName) {
		...
		//Calculate purity and NMI, treat each noise as a cluster
		if(!noise.isEmpty()){
			int size=noise.size();
			DataPoints[] noises = new DataPoints[(int) size]; 
			noise.toArray(noises);
			for(int i=0;i<noises.length;i++){
				//create a noise cluster
				Set<DataPoints> noiseCluster = new HashSet<DataPoints>();
				noiseCluster.add(noises[i]);
				
				// add to the whole clusters
		...
		//List clusters
		System.out.println("Number of clusters formed :"+(clusters.size()-noise.size()));
		System.out.println("Noise clusters :"+noise.size());
		...
		// write all the clusters including the noise
		DataPoints.writeToFile(clusters, "output//DBSCAN_result_on_"+dataSetName+".csv");
